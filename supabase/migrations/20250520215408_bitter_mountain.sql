...
LEFT JOIN 
  stripe_subscriptions s ON sc.customer_id = s.customer_id;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'user_credits' AND policyname = 'Users can view their own credit information'
  ) THEN
    EXECUTE $POLICY$
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_credits' AND policyname = 'System can update credit information'
  ) THEN
    EXECUTE $POLICY$
...
-- Drop existing trigger if it exists to avoid duplicate trigger errors
DROP TRIGGER IF EXISTS initialize_credits_on_subscription ON stripe_subscriptions;

-- Create trigger to initialize credits when a subscription is activated
...
BEGIN
  ...
  WHERE uc.user_id = u.id
    AND (s.status IS NULL OR 
         s.status NOT IN ('active', 'trialing'))
...
-- Create trigger to handle credit rollover on subscription renewal
...
-- Create trigger to give free credits to new users
