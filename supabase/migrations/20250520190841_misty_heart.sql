...
BEGIN
  ...
  BEGIN
    ...
    IF EXISTS (SELECT 1 FROM user_credits WHERE user_id = user_uuid) THEN
      ...
      SET 
        basic_credits = 
          ...
          LEAST(
            (SELECT COALESCE(basic_credits, 0) FROM user_credits WHERE user_id = user_uuid),
            max_rollover
          ),
        premium_credits = 
          -- Start with new allocation
          ...
          -- Add rollover credits (limited by max_rollover)
          LEAST(
            (SELECT COALESCE(premium_credits, 0) FROM user_credits WHERE user_id = user_uuid),
            max_rollover
          ),
