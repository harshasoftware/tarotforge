/*
  ...
  This migration adds detailed documentation to tables, views, columns, 
  triggers, and functions related to the deck quota and generation system.
  
  1. Table Documentation
     - Documents all tables in the deck quota system
     - Explains the purpose of each table
  
  2. Column Documentation
     - Documents all columns with detailed descriptions
     - Explains data types and constraints
  
  3. View Documentation
     - Documents views that combine data from multiple tables
  
  4. Trigger Documentation
     - Documents triggers that maintain data consistency
  
  5. Function Documentation
     - Safely documents functions using dynamic SQL to avoid errors
  
  6. Schema Documentation
     - Provides overview of the deck-based pricing model
*/
...
-- Add comments to functions (using dynamic SQL with proper escaping)
DO $$
DECLARE
...
BEGIN
  ...
  IF function_exists THEN
    EXECUTE $comment$
      COMMENT ON FUNCTION public.consume_user_deck_quotas(uuid, integer, integer, text, text) IS 
  ...
  IF function_exists THEN
    EXECUTE $comment$
      COMMENT ON FUNCTION public.initialize_user_deck_quota(uuid, text, integer, integer, integer, timestamp with time zone) IS 
  ...
  IF function_exists THEN
    EXECUTE $comment$
      COMMENT ON FUNCTION public.fix_missing_deck_quota_record(uuid) IS 
  ...
  IF function_exists THEN
    EXECUTE $comment$
      COMMENT ON FUNCTION public.purchase_regeneration_pack(uuid, uuid) IS 
