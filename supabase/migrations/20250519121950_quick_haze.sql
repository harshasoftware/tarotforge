/*
  # Add full_name field and fix trigger functions
  
  1. Changes
    - Add full_name field to users table
    - Update auto_confirm_user_email trigger function
    - Ensure validate_deck_creator function exists
*/
...
-- Check if validate_deck_creator function exists
SELECT EXISTS (
  SELECT 1 FROM pg_proc WHERE proname = 'validate_deck_creator'
) AS validate_deck_creator_exists \gset

-- Create validate deck creator function if not exists
DO $$
BEGIN
  IF NOT :'validate_deck_creator_exists' THEN
