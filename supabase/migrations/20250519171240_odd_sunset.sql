/*
  # Update reader level colors and icons
  
  1. Changes
    - Updates reader levels to use Chakra color system
    - Changes the color scheme: red, orange, yellow, green, violet
    - Updates icons to match the color scheme: flame, sparkles, sun, heart, crown
    
  2. Indexes
    - Adds indexes for pagination and sorting
*/
...
-- Make sure we have all five levels in the chakra-inspired color order
DO $$
BEGIN
...
-- Add indexes for sorting, pagination, and queries
DO $$
BEGIN
  ...
  -- Reader sorting and pagination indexes
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
  ...
  -- Add marketplace deck indexes for pagination
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'decks' AND indexname = 'decks_pagination_idx'
  ) THEN
    CREATE INDEX decks_pagination_idx ON decks(is_listed, created_at);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'decks' AND indexname = 'decks_is_free_is_nft_idx'
  ) THEN
    CREATE INDEX decks_is_free_is_nft_idx ON decks(is_free, is_nft);
  END IF;
END $$;