...
const ReadingRoom = () => {
  ...
  return (
    <Div100vh
      className={`flex flex-col select-none ${!isMobile ? 'h-screen overflow-hidden' : 'overflow-hidden'}`}
      style={isMobile ? { 
        width: '100vw',
        position: 'relative',
              ...
              >
                ...
                >
                  ...
                  {selectedLayout?.id === 'free-layout' && selectedCards.map((selectedCard: any, index: number) => (
                    ...
                    >
                      <div
                        className={`${isMobile ? 'w-20 h-30' : 'w-20 h-30 md:w-24 md:h-36'} shadow-lg cursor-pointer transition-shadow p-0.5 rounded-md overflow-hidden ${
                      ...
                      >
                        {(selectedCard as any).revealed ? (
                          <img
                            ...
                            className={`w-full h-full object-cover rounded-md select-none ${(selectedCard as any).isReversed ? 'rotate-180' : ''}`}
                            draggable="false"
                          />
                        ) : (
                      ...
                      >
                        ...
                        {!selectedCard && (
                          <div
                            ...
                            } rounded-md flex flex-col items-center justify-center transition-colors select-none ${
                        ...
                        {selectedCard && (
                          ...
                          >
                            <div
                              className={`${isMobile ? 'w-20 h-30' : 'w-20 h-30 md:w-24 md:h-36'} shadow-lg cursor-pointer p-0.5 rounded-md overflow-hidden`}
                              style={{
                                transform: adjustedPosition.rotation ? `rotate(${adjustedPosition.rotation}deg)` : 'none',
                                transformOrigin: 'center center'
                              }}
              ...
              <div className={`relative h-full ${isMobile ? (isLandscape && !showMobileInterpretation ? 'w-3/5' : (showMobileInterpretation ? 'hidden' : 'flex-1 w-full')) : 'w-full'}`}> 
                ...
                >
                  ...
                  {selectedLayout?.id === 'free-layout' && selectedCards.map((cardData: any, index: number) => {
                    ...
                    >
                      <div className={`${isMobile ? 'w-20 h-30' : 'w-20 h-30 md:w-24 md:h-36'} rounded-md overflow-hidden shadow-lg cursor-pointer transition-shadow ${activeCardIndex === index ? 'ring-2 ring-primary shadow-xl' : ''}`}>
                        {cardData.revealed ? (
                    ...
                    <div className={`${isMobile ? 'mb-3 p-2' : 'mb-4 md:mb-6 p-2 md:p-3'} bg-muted/30 border border-border rounded-lg`}>
                      <div className={`flex ${isMobile ? 'gap-1' : 'gap-2 md:gap-3'}`}>
                        <div className={`shrink-0 ${isMobile ? 'w-8 h-12' : 'w-10 h-15 md:w-12 md:h-18'} p-0.5`}>
                          <img src={(selectedCards[activeCardIndex] as Card).image_url} alt={(selectedCards[activeCardIndex] as Card).name} className={`w-full h-full object-cover rounded-md select-none ${(selectedCards[activeCardIndex] as any).isReversed ? 'rotate-180' : ''}`} draggable="false" />
                        </div>
                        <div className="min-w-0 flex-1">
