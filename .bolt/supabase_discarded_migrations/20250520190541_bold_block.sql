/*
  # Credit System and Subscription Integration
  
  1. New Tables
    - `user_credits` - Tracks user credit balances and details
    - `credit_transactions` - Audit log for all credit-related activities
  
  2. Schema Updates
    - Add `last_card_generation_date` to the users table
  
  3. Views
    - `user_subscription_credits` - Combined view of user subscription and credit data
  
  4. Functions
    - `initialize_user_credits()` - Set up credits when subscription changes
    - `consume_user_credits()` - Safely consume credits for card generation
    - `reset_free_tier_credits()` - Monthly credit refresh for free users
    - `handle_credit_rollover()` - Manage credit expiration at renewal
    - `initialize_free_credits()` - Give initial credits to new users
*/
...
-- Security policies for user_credits
...
-- Security policies for credit_transactions
